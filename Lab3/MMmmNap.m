...
function result = MMmmNap(tasa_llegadas, tiempo_servicio, servidores, poblacion, min_eventos)
 ...
 tiempo_total=0;
 tiempo_usuarios=zeros(1,poblacion);
  ...
  while (num_salidas < min_eventos || en_servicio > 0)
    ...
    if (siguiente_llegada < siguiente_salida)
      ...
      cambio=siguiente_llegada - tiempo_usuarios(en_servicio+1);
      tiempo_total = tiempo_total + cambio;
      tiempo_usuarios(en_servicio+1) = siguiente_llegada;
    else
      evento = SALIDA;
    ...
    %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% Trafico cursado
    ac_Tc =ac_Tc + en_servicio * intervalo;
    %% Tiempo de permanencia en el sistema
    ac_t = ac_t + en_servicio * intervalo;
    %% Usuarios en el sistema
    ac_n = ac_n + en_servicio * intervalo;
    %% Tiempo de permanencia en el estado correspondiente
    ac_pn(en_servicio + 1) = ac_pn(en_servicio + 1) + intervalo;
    ...
    if (evento == LLEGADA)
      ...
      if (en_servicio < servidores)
        ...
        ac_Tp = ac_Tp + t_servicio;
      end
      %% Programamos la siguiente llegada
      t_llegada = exprnd(1/(tasa_llegadas * (poblacion - en_servicio)));
