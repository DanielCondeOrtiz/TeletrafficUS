...
function result = MMmmAleat(tasa_llegadas,tiempo_servicio,servidores,min_tiempo)
  ...
  while (tiempo_simulado < min_tiempo || usuarios_en_servicio > 0)
    ...
    if (evento == LLEGADA)
      ...
      if (usuarios_en_servicio < servidores)
        ...
        indice2=buscaLibre(siguientes_salidas);
        siguientes_salidas(indice2) = tiempo_simulado + t_servicio;
        %% %%%%% Acumulamos los tiempos de servicio
        tserv_medido=tserv_medido + t_servicio;
        factor_oc(indice2)=factor_oc(indice2)+t_servicio;
        %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     end
 ...
 (factor_oc/tiempo_simulado)'];
    ...
    disp('Probabilidad de estados')
        disp(result(6:end-servidores))
    disp('Factor de ocupacion')
        disp(result(end-servidores+1:end))
end
...
function indice = buscaLibre(salidas)
    indices = find(salidas==Inf);
    i = round(rand(1,1)*length(indices) + 0.5);
    indice = indices(i);
end